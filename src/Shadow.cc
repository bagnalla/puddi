...
namespace puddi
{
  namespace Shadow
  {
	...
	for (auto it = depthRenderList.begin(); it != depthRenderList.end(); ++it)
      ...
      mat4 computeOrthoCamAndProjection(Scene *scene,
	...
	mat4 inverseProj = inverse(scene->GetProjection() * scene->GetView());
	...
	// cam = lookAt(vec3(centroid) + normalize(lightDir) * engine::ViewDistance * 1.25f,
	cam = lookAt(vec3(centroid) + normalize(lightDir) * engine::WorldSize * 1.5f,
		     vec3(centroid), vec3(0.0f, 0.0f, 1.0f));
    ...
    void RenderShadowOrthoMap(Scene *scene, const vec3& lightDir)
    {
      ...
      mat4 cam;
      mat4 proj = computeOrthoCamAndProjection(scene, lightDir, cam);
      ...
      // Shader::SetView(engine::CameraMatrix);
    }

    void RenderShadowCubeMap(Scene *scene, const vec3& sourcePos, DrawableObject* source)
    {
      Shader::SetProgram("depth");
	...
	source->DisableRender();
      ...
      mat4 projection = perspective(static_cast<float>(M_PI) / 2.0f, 1.0f,
				    zRange.x, zRange.y);
      Shader::SetProjection(projection);

      glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
			     GL_TEXTURE_CUBE_MAP_POSITIVE_Y, depthCubeMap, 0);
      glClear(GL_DEPTH_BUFFER_BIT);
      Shader::SetView(lookAt(sourcePos, sourcePos + vec3(0.0f, 1.0f, 0.0f), 
			       vec3(0.0f, 0.0f, 1.0f)));
      renderDepthRenderList();

      glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
			     GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, depthCubeMap, 0);
      glClear(GL_DEPTH_BUFFER_BIT);
      Shader::SetView(lookAt(sourcePos, sourcePos + vec3(0.0f, -1.0f, 0.0f),
			       vec3(0.0f, 0.0f, -1.0f)));
      renderDepthRenderList();
      ...
      glClear(GL_DEPTH_BUFFER_BIT);
      Shader::SetView(lookAt(sourcePos, sourcePos + vec3(1.0f, 0.0f, 0.0f),
			       vec3(0.0f, -1.0f, 0.0f)));
      renderDepthRenderList();

      glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
			     GL_TEXTURE_CUBE_MAP_NEGATIVE_X, depthCubeMap, 0);
      glClear(GL_DEPTH_BUFFER_BIT);
      Shader::SetView(lookAt(sourcePos, sourcePos + vec3(-1.0f, 0.0f, 0.0f),
			       vec3(0.0f, -1.0f, 0.0f)));
      renderDepthRenderList();
      ...
      glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
			     GL_TEXTURE_CUBE_MAP_POSITIVE_Z, depthCubeMap, 0);
      glClear(GL_DEPTH_BUFFER_BIT);
      Shader::SetView(lookAt(sourcePos, sourcePos + vec3(0.0f, 0.0f, 1.0f),
			       vec3(0.0f, -1.0f, 0.0f)));
      renderDepthRenderList();

      glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
			     GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, depthCubeMap, 0);
      glClear(GL_DEPTH_BUFFER_BIT);
      Shader::SetView(lookAt(sourcePos, sourcePos + vec3(0.0f, 0.0f, -1.0f),
			       vec3(0.0f, -1.0f, 0.0f)));
      renderDepthRenderList();
