...
namespace puddi
{
  namespace EnvironmentMap
  {
    ...
    void Render(Scene *scene, GLuint targetCubeMap, const vec3& sourcePos,
		DrawableObject* source, float zNear, float zFar)
    {
	...
	source->DisableRender();
      ...
      Shader::SetView(lookAt(sourcePos, sourcePos + vec3(0.0f, 1.0f, 0.0f), vec3(0.0f, 0.0f, 1.0f)));
      // engine::RenderAll();
      scene->Draw();
      ...
      glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
      Shader::SetView(lookAt(sourcePos, sourcePos + vec3(1.0f, 0.0f, 0.0f), vec3(0.0f, -1.0f, 0.0f)));
      // engine::RenderAll();
      scene->Draw();
      ...
      glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
      Shader::SetView(lookAt(sourcePos, sourcePos + vec3(0.0f, 0.0f, 1.0f), vec3(0.0f, -1.0f, 0.0f)));
      // engine::RenderAll();
      scene->Draw();
